...
export default function ChatPage() {
  ...
  const fetchMessages = async () => {
    ...
    try {
      ...
      // Set current user ID and other username from first message
      if (data.length > 0) {
        const firstMsg = data[0];
        const userId = firstMsg.senderId === otherUserId ? firstMsg.receiverId : firstMsg.senderId;
            ...
            <div className="bg-white/5 border-b border-white/20 px-6 py-4 flex items-center justify-between">
              <div className="flex items-center gap-4">
                ...
                <div className="flex items-center gap-3">
                  ...
                  <h1 className="text-xl font-bold text-white">{otherUsername}</h1>
                </div>
              </div>
            </div>
